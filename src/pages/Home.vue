...
import { normalizeUrl } from "@/utils/validate";
...
const handleSearch = async () => {
  ...
  // 如果输入被规范化了（如自动补全 https://），同步到输入框
  if (normalized !== trimmed) {
    url.value = normalized;
  }
  const data = await videoStore.fetchVideoInfo(normalized);
  if (data) {
    historyStore.add(normalized, data.videoInfo.title);
    ...
    <n-flex vertical align="center" justify="center" :size="20" class="search-view">
      <n-flex vertical align="center" :size="8">
        <n-flex align="center" class="hero-logo">
          <span class="hero-text">抖音下载器</span>
        </n-flex>
